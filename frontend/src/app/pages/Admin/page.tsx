...
interface Train {
  ...
  trainName: string;
  trainNumber: string;
  departCity: string; // Changed from departureCity
  arrivalCity: string;
  departTime: string; // Changed from departureTime
  arrivalTime: string;
  duration?: string; // Added to match DTO
}
  ...
  const fetchTrains = async () => {
    try {
      ...
      const normalizedTrains = data.map((train: Train) => ({
        ...
        departCity: train.departCity || "", // Changed from departureCity
        arrivalCity: train.arrivalCity || "",
        departTime: train.departTime || "", // Changed from departureTime
      ...
      const response = await fetch(
          ...
          : "http://localhost:3009/trains",
        {
          ...
          body: JSON.stringify({
            ...
            departTime: formTrain.departTime,
            arrivalTime: formTrain.arrivalTime,
            duration: formTrain.duration || undefined, // Send undefined if empty
          }),
        }
      );
